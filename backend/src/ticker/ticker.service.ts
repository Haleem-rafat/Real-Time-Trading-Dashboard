...
export class TickerService implements OnModuleInit {
  ...
  async onModuleInit() {
    ...
    // Note: the historical drift-heal step that used to run here has
    // been removed. It was only useful for the one-time cleanup of the
    // legacy drift-less backfill; running it on every boot was
    // catastrophic because high-volatility symbols (BTC/ETH with 3%+
    // per-tick volatility) can legitimately drift >15% from base_price
    // over a long live session even with mean reversion, which caused
    // the heal to wipe hundreds of thousands of real live-tick points
    // on every restart. If we ever need another data migration we'll
    // gate it behind an explicit version marker instead.
